...
The public algorithm name is **TRAIn-MF**. Its stable API/CLI identifier remains `MF-AUTO`; the MATLAB entry point remains `DOSY_MF_Auto`. These identifiers invoke the same numerical reference.
...
| max_components | Atomic search cap 1–4; default 4. It is a limit, not a supplied rank. TRAIn-MF uses the frozen cap of 4. |
...
TRAIn-MF output has D_grid, X, S and A with X=S A; its predictive factor count is not the number of molecular species. It reports status, success and the KKT residual. A returned unresolved result must not be reported as a validated fit.
